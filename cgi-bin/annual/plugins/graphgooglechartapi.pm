...
sub Graph_Monthly(){
	...
	$chxl .= "1:|0|".Get_Suffixed((@$valmax[2]/4),0)."|".Get_Suffixed((@$valmax[2]/2),0)."|".Get_Suffixed((@$valmax[2]*3/4),0)."|".Get_Suffixed(@$valmax[2],0)."|";
	# get bytes
	$chxl .= "2:|0|".Get_Suffixed((@$valmax[4]/4),1)."|".Get_Suffixed((@$valmax[4]/2),1)."|".Get_Suffixed((@$valmax[4]*3/4),1)."|".Get_Suffixed(@$valmax[4],1);
	# TODO add the year at the start and end
	
	# set the axis colors
	$chxs .= "1,".@$valcolor[2]."|2,".@$valcolor[4];#."|3,".@$valcolor[4];
