...
EOF
	...
	print "<script type=\"text\/javascript\" src=\"../js/tablesortandDD.js\"><\/script>"; #BG Added for sorting JS capability
...
sub HTMLTopBanner{
	...
	#Back to original code for top of page, only for monthly pages
...
"<tr><td class=\"aws\" valign=\"middle\"><b>$Message[7]:</b>&nbsp;</td><td class=\"aws\" valign=\"middle\"><span style=\"font-size: 14px;\">$SiteDomain</span></td>";
		...
		if ( $FrameName ne 'mainleft' ) {
			if ( $LogoLink =~ "http://www.awstats.org" ) {
				...
				#print "<td align=\"right\" rowspan=\"3\"><a href=\""
                print "<td align=\"right\" rowspan=\"3\"><a "
				  . XMLEncode($LogoLink)
...
"<a href=\"$linkanchor#top\"$targetpage>$Message[128]</a>";
			...
			#HTMLShowMenuCateg( #BG remove this menu that shows link to the Full List tables ( Who: Links, etc)
			#	'who',          $Message[92],
		#		'menu5.png',    $frame,
		#		$targetpage,    $linkanchor,
		#		$NewLinkParams, $NewLinkTarget,
...
"<a href=\"$linkanchor#sessions\"$targetpage>$Message[117]</a>";
            ...
            #BG remove this section from nav. menu at top of page manually
				#print( $frame? "<tr><td class=\"awsm\">" : "" );
				#print
...
"<a href=\"$linkanchor#screensizes\"$targetpage>$Message[135]</a>";
	...
	# Print Back link
			...
			|| $QueryString =~ /buildpdf/i )
		{
			...
			print
#"<tr><td class=\"aws\"><a href=\"javascript:parent.window.close();\">$Message[118]</a></td></tr>\n"; #BG Removed Close Window Link
"<tr><td class=\"aws\"><a href=\"\"> </a></td></tr>\n"; #BG Added for removing Close Window Link
		}
		print "</table>\n";
		print "\n";
	}
...
sub HTMLMainCountries{
	...
	my $title =
#"$Message[25] ($Message[77] $MaxNbOf{'Domain'}) &nbsp; - &nbsp; <a href=\"" #BG remove original with the (Top 50) listed
"$Message[25]  &nbsp; ";
    #BG remove section below containing Full List linkage for domain/countries table
	#  . (
	#	$ENV{'GATEWAY_INTERFACE'}
	#	  || !$StaticLinks
	...
	#&tab_head( "$title", 19, 0, 'countries' ); #BG remove original on monthly summary page
	&tab_headautosort3( "$title", 19, 0, 'countries' ); #BG added to auto-sort column 3 (hits) on monthly summary page
...
"<tr bgcolor=\"#$color_TableBGRowTitle\"><th width=\"$WIDTHCOLICON\">&nbsp;</th><th colspan=\"2\">$Message[17]</th>";
	...
	if ( $ShowDomainsStats =~ /H/i ) { #BG This is the table shown on monthly page, not Full List
		#print "<th bgcolor=\"#$color_h\" width=\"80\"" 
		#  . Tooltip(4)
		#  . ">$Message[57]</th>";
...
# Input:        -
# Output:       HTML
# Return:       #BG Host table in main page
#------------------------------------------------------------------------------
sub HTMLMainHosts{
	my $NewLinkParams = shift;
	my $NewLinkTarget = shift;
    ...
    print "<br /><div id=\"FrogHostTableDesc\">&nbsp;</div>"; #BG Added for Comments on Hosts Table section via JS
	print "$Center<a name=\"visitors\">&nbsp;</a><br />\n";
	my $title =
#BG Remove "$Message[81] ($Message[77] $MaxNbOf{'HostsShown'}) &nbsp; - &nbsp; <a href=\""
"$Message[81] (Full List) &nbsp;"; #BG Changed to remove all links to Full List and others
    #BG removed below codeblock to remove all links to Full List
	  # . (
		# $ENV{'GATEWAY_INTERFACE'}
		  # || !$StaticLinks
	...
	#&tab_head( "$title", 19, 0, 'visitors' ); #original on monthly summary table
	&tab_headautosort1( "$title", 19, 0, 'visitors' );
...
"$Message[81] : ".Format_Number($TotalHostsKnown)." $Message[82], ".Format_Number($TotalHostsUnknown)." $Message[1]<br />".Format_Number($TotalUnique)." $Message[11]</th>";
    ...
    #BG added change below for sort on hit column in host table
	if ( $ShowHostsStats =~ /H/i ) {
...
sub HTMLMainExtra{
	...
	if ($Debug) { debug( "ExtraName$extranum", 2 ); }
	print "$Center<a name=\"extra$extranum\">&nbsp;</a><br />";
    print "<div id=\"extra$extranum\">&nbsp;</div><br />"; #BG Added for Comments on each extra section via JS
	my $title = $ExtraName[$extranum];
	&tab_head( "$title", 19, 0, "extra$extranum" );
	print "<tr bgcolor=\"#$color_TableBGRowTitle\">";
...
if ( scalar keys %HTMLOutput ) {
	...
	if ($DetailedReportsOnNewWindows) {
        ...
        $NewLinkTarget = " target=\"_self\""; #BG Change to open the Full Details Pages in same tab hyperlink
	}
...
"firstdaytoshowtime=$firstdaytoshowtime, lastdaytoshowtime=$lastdaytoshowtime",
	...
	if ( $HTMLOutput{'main'} ) {
		...
		#--------------------------
		#BGif ($ShowHostsStats) {
		#BG	&HTMLMainHosts($NewLinkParams, $NewLinkTarget);
		#BG}
        ...
        #BG remove this table manually, if we turn it off in config file, download hits won't be shown
